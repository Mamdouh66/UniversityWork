...
public class Main {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        if(userInterface == 1){ // if he chose the admin interface
            ...
            boolean check = admin.loginInterface(1); // if true, enters the interface
            while(!check){ // if false it will loop until the user logs in
                check = admin.loginInterface(1);
            }
            int choice;
            do {
            ...
            boolean check = customer.loginInterface(2);
            while(!check) {
                check = customer.loginInterface(2);
            }
            int choice = 0;
            do{
